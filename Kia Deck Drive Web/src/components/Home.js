...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const persNo = localStorage.getItem('persNo');
      console.log("PersNo obtenido del localStorage:", persNo);  // Verificar si el persNo se obtuvo correctamente
  
      if (!persNo) {
        console.error("No se encontró el Pers.No. en localStorage");
        setLoading(false);
        return;
      }
  
      try {
        const response = await fetch(`http://localhost:5000/api/usuarios/${persNo}`);
        console.log("Response status:", response.status);  // Verificar el estado de la respuesta HTTP
  
        if (response.ok) {
          const data = await response.json();
          console.log("Datos obtenidos del servidor:", data);  // Verificar si los datos son correctos
          setUsuario(data);  // Almacena los datos del usuario en el estado
        } else {
          console.error('Error en la respuesta:', await response.json());  // Verificar los errores del backend
        }
      } catch (error) {
        console.error('Error al obtener los datos del servidor:', error);  // Manejo de error de red
      } finally {
        setLoading(false);  // Asegúrate de que siempre deje de cargar
      }
    };
  ...
  // Verifica si datosUsuario no es nulo antes de intentar acceder a sus propiedades
if (!usuario) {
  return (
    <>
    <main>
    <div class="container">
      <div class="card mt-5">
        <div
          ...
          style={{
            backgroundImage:
              'url(https://prium.github.io/falcon/v3.22.0/assets/img/icons/spot-illustrations/corner-4.png)',
          }}
        ></div>
        ...
        <div class="card-body position-relative">
          <div class="row g-2 align-items-sm-center">
            <div class="col-auto">
              <img
                src="../assets/img/icons/connect-circle.png"
                alt=""
                height="55"
              />
            </div>
            <div class="col">
              <div class="row align-items-center">
                <div class="col col-lg-8">
                  <h5 class="fs-9 mb-3 mb-sm-0 text-primary">
                    No se encontraron datos del usuario. Registrate
                  </h5>
                </div>
                <div class="col-12 col-sm-auto ms-auto">
                  <Link to="/juego" class="btn btn-falcon-primary" type="button">
                    Correr juego
                  </Link>
                </div>
              </div>
                  ...
                  <div className="card font-sans-serif">
                    <div className="card-body d-flex gap-3 flex-column flex-sm-row align-items-center"><img className="rounded-3" src="https://static-00.iconduck.com/assets.00/user-icon-512x512-x23sj495.png" alt="" width="112" />
                      <table className="table table-borderless fs-10 fw-medium mb-0">
                        <tbody>
                          <tr>
                            <td className="p-1" StyleName={{ width: '35%' }}>Nombre:</td> {usuario.nombre}
                            <td className="p-1 text-600">{usuario.nombre}</td>
                          </tr>
                          <tr>
                            <td className="p-1" StyleName={{ width: '35%' }}>Registro en:</td> { usuario['Registro en'] }
                            <td className="p-1 text-600">{usuario['Registro en']}</td>
                          </tr>
                          <tr>
                            <td className="p-1" StyleName={{ width: '35%' }}>Grupo:</td> { usuario.grupo }
                            <td className="p-1"><a className="text-600 text-decoration-none">{usuario.grupo} </a><small className="badge rounded badge-subtle-success false">Verificado</small></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div className="card font-sans-serif">
                    ...
                    <div className="card-body">
                      <div className="row g-0">
                        <div className="col-6"> 
                          <h4 className="text-700 lh-1 mb-1"> </h4><small className="badge rounded badge-subtle-warning false">Histórico</small>{usuario.TiempoTotalJugado || 0}
                        </div>
                ...
                <div className="col-md-6">
                  <div className="card font-sans-serif">
                    ...
                    <div className="card-body">
                      <div className="row g-0">
                        <div className="col-6"> 
                          <h4 className="text-700 lh-1 mb-1"></h4><small className="badge rounded badge-subtle-warning false">Histórico</small> {usuario.PuntajeTotal || 0}
                        </div>
                        <div className="col-6 mt-n4 d-flex justify-content-end">
                    ...
                    <div className="col-sm-7 order-1 order-sm-0">
                      <div className="row g-sm-0 gy-4 row-cols-2 h-100 align-content-between">
                        <div className="col">
                          ...
                          <h5 className="fw-normal">{usuario && usuario['Pers.No.'] ? usuario['Pers.No.'] : '0'}</h5>
                          <h6 className="mb-0"><span className="text-500 me-2">Personal y no transferible</span><small className="badge rounded badge-subtle-success false">2.1%</small></h6>
                        </div>
                        <div className="col">
                          <div className="d-flex gap-2 mb-3">
                            <div className="vr rounded ps-1 bg-primary"></div>
                            <h6 className="lh-base text-700 mb-0">Puntaje de la última partida</h6>
                          </div>
                          <h5 className="fw-normal"> {usuario && usuario.UltimaPuntuacion ? usuario.UltimaPuntuacion : '0'}</h5>
                        ...
                        <div className="col">
                          ...
                          <h5 className="fw-normal"> {usuario.PartidasTotales || 0}</h5>
                        ...
                        <div className="col">
                          ...
                          <h5 className="fw-normal">{usuario ? usuario.Logros : '0'}</h5>
